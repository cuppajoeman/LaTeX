...
\begin{thm}[n distinct eigen values ]\index{n distinct eigen values }\label{thm:n_distinct_eigen_values_}
    ...
    \]
    where $\vec{v} _{i} $ is an eigen vectors with eigen values $\lambda _{i} $ is a basis for $\mathbb{R} ^{n} $ \todo{prove this} 
...
\begin{eg}
    ...
    \begin{itemize}
        ...
        \item There exists a transformation $T$ such that $A$ induces it in the standard basis. That is 
            ...
            \]
            then we have \todo{how come we can do this step?} 
            \[
